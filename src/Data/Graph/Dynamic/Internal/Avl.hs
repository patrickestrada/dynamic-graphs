...
assertInvariants t = do
  ...
  where
    ...
    computeAggs p x = do
        ...
        when (actualAggs /= storedAggs) $
            error $
                "error in stored aggregates: "
                    ++ show storedAggs
                    ++ ", actual: "
                    ++ show actualAggs
