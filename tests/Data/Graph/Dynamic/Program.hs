  ...
  where
    genInsert =
        let (v, vs1) = case vs0 of
            ...
            graph1 = Slow.insert v graph0
         in return $ Just (Insert v, graph1, vs1)

    genLink = do
        x <- QC.elements $ Slow.vertices graph0
        y <- QC.elements $ Slow.vertices graph0 \\ [x]
        if
            | Slow.connected x y graph0 && acyclic ->
                return Nothing
            | Slow.edge x y graph0 ->
                return Nothing
            | otherwise ->
                let graph1 = Slow.link x y graph0
                 in return $ Just (Link x y, graph1, vs0)
